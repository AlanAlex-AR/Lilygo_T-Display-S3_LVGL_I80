...
{
    String text;
    lv_obj_t *logo_gif = lv_gif_create(lv_scr_act());
    lv_obj_center(logo_gif);
    lv_gif_set_src(logo_gif, &lilygo2_gif);
    LV_DELAY(1200);

    // Funcional 
    // lv_obj_t *label = lv_label_create( lv_scr_act() );
    // lv_label_set_text( label, "Hello Como estamos Galletitas!");
    // lv_obj_align( label, LV_ALIGN_CENTER, 0, 0 );

    // Funcional 
    // lv_obj_t *logo_imagen = lv_img_create(lv_scr_act());
    // lv_obj_align(logo_imagen, LV_ALIGN_CENTER, 0, 0);
    // lv_img_set_src(logo_imagen, &logo_img);

    // // lv_obj_t *log_label = lv_label_create(lv_scr_act());
    // // lv_obj_align(logo_gif, LV_ALIGN_CENTER, 0, 0);
    // // lv_obj_set_width(log_label, LV_PCT(100));
    // // lv_label_set_long_mode(log_label, LV_LABEL_LONG_SCROLL);
    // // lv_label_set_recolor(log_label, true);

    // // lv_label_set_text(log_label, "Hello");
    // LV_DELAY(1);
